...
namespace ExcelReporter.Tests.Implementations.Panels.Excel.PanelRenderTests
{
    ...
    public class PanelRenderTest
    {
        ...
        public void TestNamedPanelRender()
        {
            ...
            ws.Cell(10, 2).Value = "''{m:Format(ms:AddDays(p:DateParam, p:IntParam), \"yyyy-MM-dd\")}";
            ws.Cell(10, 3).Value = "''{m:Format(ms:AddDays(p:DateParam, 5), ddMMyyyy)}";
            ws.Cell(10, 4).Value = "''{m:Format(ms:AddDays(p:DateParam, -2), dd.MM.yyyy)}";
            ws.Cell(10, 5).Value = "''{m:Format(ms:AddDays(p:DateParam, [int]-3), \"dd.MM.yyyy HH:mm:ss\")}";
