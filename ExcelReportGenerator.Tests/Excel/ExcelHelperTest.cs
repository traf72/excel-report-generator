...
public class ExcelHelperTest
{
    ...
    public void TestGetNearestParentRange()
    {
        ...
        var parentRange1 = ws.Range(1, 1, 7, 7);
        var childRange = ws.Range(7, 7, 7, 7);
        Assert.AreEqual(parentRange1, ExcelHelper.GetNearestParentRange(new[] {parentRange1}, childRange));

        var parentRange2 = ws.Range(2, 2, 7, 7);
        Assert.AreEqual(parentRange2,
            ExcelHelper.GetNearestParentRange(new[] {parentRange1, parentRange2}, childRange));

        var notParentRange = ws.Range(2, 2, 7, 6);
        Assert.AreEqual(parentRange2,
            ExcelHelper.GetNearestParentRange(new[] {parentRange1, parentRange2, notParentRange}, childRange));

        var parentRange3 = ws.Range(7, 7, 7, 7);
        Assert.AreEqual(parentRange3,
            ExcelHelper.GetNearestParentRange(new[] {parentRange1, parentRange2, notParentRange, parentRange3},
                childRange));

        var parentRange4 = ws.Range(7, 7, 7, 7);
        ExceptionAssert.Throws<InvalidOperationException>(
            () => ExcelHelper.GetNearestParentRange(
                new[] {parentRange1, parentRange2, notParentRange, parentRange3, parentRange4}, childRange),
            "Found more than one nearest parent ranges");

        var badChildRange = ws.Range(7, 7, 7, 8);
        ExceptionAssert.Throws<InvalidOperationException>(
            () => ExcelHelper.GetNearestParentRange(
                new[] {parentRange1, parentRange2, notParentRange, parentRange3, parentRange4}, badChildRange),
    ...
    public void TestGetCellCoordsRelativeRange()
    {
        ...
        var cell1 = ws.Cell(1, 1);
        var cell2 = ws.Cell(7, 8);
        ExceptionAssert.Throws<InvalidOperationException>(() => ExcelHelper.GetCellCoordsRelativeRange(range, cell1),
            $"Cell {cell1} is outside of the range {range}");
        ExceptionAssert.Throws<InvalidOperationException>(() => ExcelHelper.GetCellCoordsRelativeRange(range, cell2),
            $"Cell {cell2} is outside of the range {range}");
    ...
    public void TestGetRangeCoordsRelativeParent()
    {
        ...
        var parentRange = ws.Range(3, 3, 6, 7);
        var childRange = ws.Range(5, 6, 5, 7);
        Assert.AreEqual(new RangeCoords(new CellCoords(3, 4), new CellCoords(3, 5)),
            ExcelHelper.GetRangeCoordsRelativeParent(parentRange, childRange));

        childRange = ws.Range(3, 3, 6, 7);
        Assert.AreEqual(new RangeCoords(new CellCoords(1, 1), new CellCoords(4, 5)),
            ExcelHelper.GetRangeCoordsRelativeParent(parentRange, childRange));

        childRange = ws.Range(3, 3, 3, 3);
        Assert.AreEqual(new RangeCoords(new CellCoords(1, 1), new CellCoords(1, 1)),
            ExcelHelper.GetRangeCoordsRelativeParent(parentRange, childRange));

        childRange = ws.Range(6, 7, 6, 7);
        Assert.AreEqual(new RangeCoords(new CellCoords(4, 5), new CellCoords(4, 5)),
            ExcelHelper.GetRangeCoordsRelativeParent(parentRange, childRange));

        childRange = ws.Range(4, 4, 6, 5);
        Assert.AreEqual(new RangeCoords(new CellCoords(2, 2), new CellCoords(4, 3)),
            ExcelHelper.GetRangeCoordsRelativeParent(parentRange, childRange));

        childRange = ws.Range(4, 4, 7, 5);
        ExceptionAssert.Throws<InvalidOperationException>(
            () => ExcelHelper.GetRangeCoordsRelativeParent(parentRange, childRange),
            $"Range {parentRange} is not a parent of the range {childRange}. Child range is outside of the parent range.");

        Assert.AreEqual(new RangeCoords(new CellCoords(2, 2), new CellCoords(5, 3)),
            ExcelHelper.GetRangeCoordsRelativeParent(parentRange, childRange, false));

        childRange = ws.Range(1, 1, 2, 1);
        Assert.AreEqual(new RangeCoords(new CellCoords(-1, -1), new CellCoords(0, -1)),
            ExcelHelper.GetRangeCoordsRelativeParent(parentRange, childRange, false));
        ...
        Assert.AreEqual(new RangeCoords(new CellCoords(6, 4), new CellCoords(7, 5)),
            ExcelHelper.GetRangeCoordsRelativeParent(parentRange, childRange, false));
    }
    ...
    public void TestCopyRange()
    {
        ...
        var newRange = ExcelHelper.CopyRange(range, ws.Cell(10, 8));
        ...
        range.Clear();
        var newRange2 = ExcelHelper.CopyRange(newRange, ws.Cell(11, 8));
    ...
    public void TestCopyNamedRange()
    {
        ...
        var copiedNamedRange = ExcelHelper.CopyNamedRange(namedRange, ws.Cell(10, 8), "Copy");
        var newRange = copiedNamedRange.Ranges.ElementAt(0);
        ...
        range.Clear();
        var copiedNamedRange2 = ExcelHelper.CopyNamedRange(copiedNamedRange, ws.Cell(11, 8), "Copy2");
        var newRange2 = copiedNamedRange2.Ranges.ElementAt(0);
    ...
    public void TestAllocateSpaceForNextRange()
    {
        ...
        var rangeStartCell = ws.Cells().Single(c => c.Value.ToString() == "RangeStart");
        var rangeEndCell = ws.Cells().Single(c => c.Value.ToString() == "RangeEnd");
    ...
    public void TestMoveRange()
    {
        ...
        var movedRange = ExcelHelper.MoveRange(range, ws.Cell(10, 8));
        ...
        var movedRange2 = ExcelHelper.MoveRange(movedRange, ws.Cell(11, 8));
    ...
    public void TestMergeRanges()
    {
        ...
        var ws = wb.AddWorksheet("Test");
        var ws2 = wb.AddWorksheet("Test2");
        var range1 = ws.Range(3, 3, 5, 5);
        var range2 = ws.Range(1, 7, 2, 8);
        var range3 = ws2.Range(1, 7, 2, 8);
    ...
    public void TestGetMaxCell()
    {
        ...
        Assert.AreEqual(ws.Cell(10, 10),
            ExcelHelper.GetMaxCell(range.Cells().Concat(new[] {ws.Cell(10, 10)}).ToArray()));
