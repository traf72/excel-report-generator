...
public class ExcelAssert
{
    public static void AreWorksheetsContentEquals(IXLWorksheet expected, IXLWorksheet actual)
    {
        ...
        Assert.AreEqual(expected.CellsUsed(XLCellsUsedOptions.All).Count(),
            actual.CellsUsed(XLCellsUsedOptions.All).Count(), "Cells used count failed");

        var expectedFirstCellUsed = expected.FirstCellUsed(XLCellsUsedOptions.All);
        var actualFirstCellUsed = actual.FirstCellUsed(XLCellsUsedOptions.All);
        Assert.AreEqual(expectedFirstCellUsed.Address, actualFirstCellUsed.Address, "First cell used failed");
        var expectedLastCellUsed = expected.LastCellUsed(XLCellsUsedOptions.All);
        var actualLastCellUsed = actual.LastCellUsed(XLCellsUsedOptions.All);
        Assert.AreEqual(expectedLastCellUsed.Address, actualLastCellUsed.Address, "Last cell used failed");

        var range = expected.Range(expectedFirstCellUsed, expectedLastCellUsed);
        foreach (var expectedCell in range.Cells())
        {
            var actualCell = actual.Cell(expectedCell.Address);
            if (expectedCell.HasFormula)
            {
                Assert.AreEqual(expectedCell.FormulaA1, actualCell.FormulaA1,
                    $"Cell {expectedCell.Address} FormulaA1 failed.");
                Assert.AreEqual(expectedCell.FormulaR1C1, actualCell.FormulaR1C1,
                    $"Cell {expectedCell.Address} FormulaR1C1 failed.");
                Assert.AreEqual(expectedCell.FormulaReference, actualCell.FormulaReference,
                    $"Cell {expectedCell.Address} FormulaReference failed.");
            }
            else
            {
    ...
    private static void AreMergedRangesEquals(IXLRanges expected, IXLRanges actual)
    {
        ...
        {
            Assert.AreEqual(expectedArray[i].RangeAddress.FirstAddress.RowNumber,
                actualArray[i].RangeAddress.FirstAddress.RowNumber, "Merge range first address row number failed");
            Assert.AreEqual(expectedArray[i].RangeAddress.FirstAddress.ColumnNumber,
                actualArray[i].RangeAddress.FirstAddress.ColumnNumber,
                "Merge range first address column number failed");
            Assert.AreEqual(expectedArray[i].RangeAddress.LastAddress.RowNumber,
                actualArray[i].RangeAddress.LastAddress.RowNumber, "Merge range last address row number failed");
            Assert.AreEqual(expectedArray[i].RangeAddress.LastAddress.ColumnNumber,
                actualArray[i].RangeAddress.LastAddress.ColumnNumber, "Merge range last address column number failed");
        }
    }
    ...
    public static void AreNamedRangesEquals(IXLNamedRanges expected, IXLNamedRanges actual)
    {
        ...
        foreach (var expectedNamedRange in expected)
        {
            ...
            Assert.AreEqual(expectedNamedRange.Ranges.Count, actualNamedRange.Ranges.Count,
                $"Named range {expectedNamedRange.Name} ranges count failed");
            for (var i = 0; i < expectedNamedRange.Ranges.Count; i++)
            {
                var expectedRange = expectedNamedRange.Ranges.ElementAt(i);
                var actualRange = actualNamedRange.Ranges.ElementAt(i);
                Assert.AreEqual(expectedRange.FirstCell().Address, actualRange.FirstCell().Address,
                    $"Named range {expectedNamedRange.Name} range {i + 1} first cell address failed");
                Assert.AreEqual(expectedRange.LastCell().Address, actualRange.LastCell().Address,
                    $"Named range {expectedNamedRange.Name} range {i + 1} last cell address failed");
            }
        }
    }
    ...
    public static void ArePageSetupEquals(IXLPageSetup expected, IXLPageSetup actual, string message = null)
    {
        ...
        Assert.AreEqual(expected.AlignHFWithMargins, actual.AlignHFWithMargins,
            string.Format(message, "AlignHFWithMargins"));
        Assert.AreEqual(expected.CenterVertically, actual.CenterVertically, string.Format(message, "CenterVertically"));
        Assert.AreEqual(expected.CenterHorizontally, actual.CenterHorizontally,
            string.Format(message, "CenterHorizontally"));
        Assert.AreEqual(expected.DifferentFirstPageOnHF, actual.DifferentFirstPageOnHF,
            string.Format(message, "DifferentFirstPageOnHF"));
        Assert.AreEqual(expected.DifferentOddEvenPagesOnHF, actual.DifferentOddEvenPagesOnHF,
            string.Format(message, "DifferentOddEvenPagesOnHF"));
        ...
        Assert.AreEqual(expected.FirstRowToRepeatAtTop, actual.FirstRowToRepeatAtTop,
            string.Format(message, "FirstRowToRepeatAtTop"));
        Assert.AreEqual(expected.LastRowToRepeatAtTop, actual.LastRowToRepeatAtTop,
            string.Format(message, "LastRowToRepeatAtTop"));
        Assert.AreEqual(expected.FirstColumnToRepeatAtLeft, actual.FirstColumnToRepeatAtLeft,
            string.Format(message, "FirstColumnToRepeatAtLeft"));
        Assert.AreEqual(expected.LastColumnToRepeatAtLeft, actual.LastColumnToRepeatAtLeft,
            string.Format(message, "LastColumnToRepeatAtLeft"));
        Assert.AreEqual(expected.ScaleHFWithDocument, actual.ScaleHFWithDocument,
            string.Format(message, "ScaleHFWithDocument"));
        Assert.AreEqual(expected.ShowGridlines, actual.ShowGridlines, string.Format(message, "ShowGridlines"));
        Assert.AreEqual(expected.ShowRowAndColumnHeadings, actual.ShowRowAndColumnHeadings,
            string.Format(message, "ShowRowAndColumnHeadings"));
        ...
        for (var i = 0; i < expected.ColumnBreaks.Count; i++)
        {
        ...
        for (var i = 0; i < expected.RowBreaks.Count; i++)
        {
