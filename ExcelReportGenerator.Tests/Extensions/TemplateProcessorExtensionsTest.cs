...
public class TemplateProcessorExtensionsTest
{
    ...
    {
        var processor = Substitute.For<ITemplateProcessor>();
    ...
    public void TestBuildMethodCallTemplate()
    {
        ...
        Assert.AreEqual("{m:Meth(p:Name, di:Value, m:MethInner())}",
            processor.BuildMethodCallTemplate("Meth(p:Name, di:Value, m:MethInner())"));
        ...
        Assert.AreEqual("Meth(p:Name, di:Value, MethInner())",
            processor.BuildMethodCallTemplate("Meth(p:Name, di:Value, MethInner())"));
    ...
    public void TestTrimMethodCallLabel()
    {
        ...
        Assert.AreEqual("Meth1(p:Name, di:Value, m:MethInner()) m:Meth2()",
            processor.TrimMethodCallLabel("m:Meth1(p:Name, di:Value, m:MethInner()) m:Meth2()"));
    ...
    public void TestGetFullRegexPattern()
    {
        ...
        var pattern = processor.GetFullRegexPattern();
        Assert.AreEqual("\\{\\s*(p|di|m|sv|sf):.+?\\s*}", pattern);

        var matches = Regex.Matches("{p:Name}", pattern);
        ...
        matches = Regex.Matches(
            "One {p:Name} {Two} Three{di:Employee.Contacts.Phone} {n:Four} {m:Meth1(hi, 5, p:Value, m:Meth2(m:Meth3(di:Coplex.Field)))} {ms:Method()}",
            pattern);
        ...
        matches = Regex.Matches(
            "One [prop*Name] [Two] Three [data*Employee.Contacts.Phone] [n*Four] [meth*Meth1(hi, 5, prop*Value, meth*Meth2(meth*Meth3(data:Coplex.Field)))] [ms*Method()]",
            pattern);
        Assert.AreEqual(3, matches.Count);
        Assert.AreEqual("[prop*Name]", matches[0].Value);
        ...
        matches = Regex.Matches("<<meth&&Meth(prop&&Value, data&&Name, 5, \"Hi\", meth&&Meth2(meth&&Meth3()))@@",
            pattern);
        Assert.AreEqual(1, matches.Count);
        Assert.AreEqual("<<meth&&Meth(prop&&Value, data&&Name, 5, \"Hi\", meth&&Meth2(meth&&Meth3()))@@",
            matches[0].Value);

        matches = Regex.Matches(
            "One <<prop&&Name@@ <<Two@@ Three <<data&&Employee.Contacts.Phone@@ <<n&&Four@@ <<meth&&Meth1(hi, 5, prop&&Value, meth&&Meth2(meth&&Meth3(data:Complex.Field)))@@ <<ms&&Method()@@ Five <<sysv&&Now@@",
            pattern);
        Assert.AreEqual(4, matches.Count);
        Assert.AreEqual("<<prop&&Name@@", matches[0].Value);
        Assert.AreEqual("<<data&&Employee.Contacts.Phone@@", matches[1].Value);
        Assert.AreEqual("<<meth&&Meth1(hi, 5, prop&&Value, meth&&Meth2(meth&&Meth3(data:Complex.Field)))@@",
            matches[2].Value);
    ...
    public void TestGetPropertyRegexPattern()
    {
        ...
        var pattern = processor.GetPropertyRegexPattern();
    ...
    public void TestGetDataItemRegexPattern()
    {
        ...
        var pattern = processor.GetDataItemRegexPattern();
        Assert.AreEqual("\\{\\s*di:.+?\\s*}", pattern);

        var matches = Regex.Matches("{ di:Name }", pattern);
        Assert.AreEqual(1, matches.Count);
        Assert.AreEqual("{ di:Name }", matches[0].Value);
    ...
    public void TestGetMethodCallRegexPattern()
    {
        ...
        var pattern = processor.GetMethodCallRegexPattern();
        Assert.AreEqual("\\{\\s*m:.+?\\s*}", pattern);

        var matches = Regex.Matches("{m:Meth()}", pattern);
        Assert.AreEqual(1, matches.Count);
        Assert.AreEqual("{m:Meth()}", matches[0].Value);
    ...
    public void TestGetVariableRegexPattern()
    {
        ...
        var pattern = processor.GetVariableRegexPattern();
        Assert.AreEqual("\\{\\s*sv:.+?\\s*}", pattern);

        var matches = Regex.Matches("{ sv:Now }", pattern);
        Assert.AreEqual(1, matches.Count);
        Assert.AreEqual("{ sv:Now }", matches[0].Value);
    ...
    public void TestGetSystemFunctionRegexPattern()
    {
        ...
        var pattern = processor.GetSystemFunctionRegexPattern();
        Assert.AreEqual("\\{\\s*sf:.+?\\s*}", pattern);

        var matches = Regex.Matches("{ sf:Format(p:Prop, 0) }", pattern);
        Assert.AreEqual(1, matches.Count);
        Assert.AreEqual("{ sf:Format(p:Prop, 0) }", matches[0].Value);
    ...
    public void TestGetAggregationRegexPatterns()
    {
        ...
        var allAggFuncs = Enum.GetNames(typeof(AggregateFunction))
            .Where(n => n != AggregateFunction.NoAggregation.ToString()).ToArray();
        Assert.AreEqual(6, allAggFuncs.Length);

        var templatesWithAggregationPattern = processor.GetTemplatesWithAggregationRegexPattern();
        Assert.AreEqual($@"\{{[^}}]*({string.Join("|", allAggFuncs)})\(\s*di\s*:.+?\)[^}}]*}}",
            templatesWithAggregationPattern);

        var aggregationFuncPattern = processor.GetAggregationFuncRegexPattern();
        Assert.AreEqual($@"({string.Join("|", allAggFuncs)})\((\s*di\s*:.+?)\)", aggregationFuncPattern);

        var matches = Regex.Matches("{Sum(di:Amount)}", templatesWithAggregationPattern);
        Assert.AreEqual(1, matches.Count);
        Assert.AreEqual("{Sum(di:Amount)}", matches[0].Value);

        var innerMatches = Regex.Matches(matches[0].Value, aggregationFuncPattern);
        ...
        matches = Regex.Matches("{ Count( di : Amount, CustomAggregation, PostAggregation) }",
            templatesWithAggregationPattern);
        Assert.AreEqual(1, matches.Count);
        ...
        matches = Regex.Matches("Text {Plain Text} {Sum(di:Amount))} {p:Text} {Avg(di:Value)} {sv:RenderDate}",
            templatesWithAggregationPattern);
        ...
        Assert.AreEqual("{m:Meth(1, Avg( di : Value ), Min(di:Amount, CustomAggregation, PostAggregation), \"Str\"}",
            matches[2].Value);
        ...
        Assert.AreEqual($@"<[^>]*({string.Join("|", allAggFuncs)})\(\s*d\s*-.+?\)[^>]*>",
            templatesWithAggregationPattern);
    ...
    public void TestBuildAggregationFuncTemplate()
    {
        ...
        Assert.AreEqual("{Avg(di:Result.Sum)}",
            processor.BuildAggregationFuncTemplate(AggregateFunction.Avg, "Result.Sum"));
