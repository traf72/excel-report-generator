...
public class TestReport : BaseReport
{
    ...
    public DateTime DateParam { get; } = new(2017, 10, 25);

    public TimeSpan TimeSpanParam { get; set; } = TimeSpan.FromHours(20);

    public ComplexType ComplexTypeParam { get; set; } = new();
    ...
    public void AfterRenderParentDataSourcePanelChildLeft(PanelEventArgs args)
    {
        ...
        var worksheet = args.Range.Worksheet;
        var firstColumnFirstCellAddress = args.Range.FirstColumn().FirstCell().Address;
        var firstColumnLastCellAddress = args.Range.FirstColumn().LastCell().Address;

        args.Range.FirstColumn().Delete(XLShiftDeletedCells.ShiftCellsLeft);
        var range = worksheet.Range(firstColumnFirstCellAddress, firstColumnLastCellAddress);
    ...
    {
        var targetCell = args.Range.Cells().ElementAt(2);
...
public class BaseReport
{
    protected BaseReport()
    {
        ...
        TemplateProcessor = new DefaultTemplateProcessor(
            new DefaultPropertyValueProvider(typeProvider, instanceProvider), new SystemVariableProvider(),
            new DefaultMethodCallValueProvider(typeProvider, instanceProvider),
            new DefaultDataItemValueProvider(new DataItemValueProviderFactory())
            {
