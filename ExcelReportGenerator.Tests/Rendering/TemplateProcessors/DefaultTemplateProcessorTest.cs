...
public class DefaultTemplateProcessorTest
{
    ...
    public void TestGetValue()
    {
        ...
        ExceptionAssert.Throws<ArgumentNullException>(() =>
            new DefaultTemplateProcessor(null, variableValueProvider, methodCallValueProvider, dataItemValueProvider));
        ExceptionAssert.Throws<ArgumentNullException>(() =>
            new DefaultTemplateProcessor(propertyValueProvider, null, methodCallValueProvider, dataItemValueProvider));
        ...
        var templateProcessor = new DefaultTemplateProcessor(propertyValueProvider, variableValueProvider,
            methodCallValueProvider, dataItemValueProvider);

        ExceptionAssert.Throws<ArgumentException>(() => templateProcessor.GetValue(null, dataItem));
        ExceptionAssert.Throws<InvalidTemplateException>(() => templateProcessor.GetValue("{p-Name}"),
            "Invalid template \"{p-Name}\". Cannot find separator \":\" between member label and member template");
        ExceptionAssert.Throws<InvalidTemplateException>(() => templateProcessor.GetValue("{bad:Name}"),
            "Invalid template \"{bad:Name}\". Unknown member label \"bad\"");
        ...
        variableValueProvider.DidNotReceiveWithAnyArgs().GetVariable(Arg.Any<string>());
        propertyValueProvider.DidNotReceiveWithAnyArgs().GetValue(Arg.Any<string>());
        methodCallValueProvider.Received(1)
            .CallMethod("Format(5567.88, 0)", typeof(SystemFunctions), templateProcessor, null);
        dataItemValueProvider.DidNotReceiveWithAnyArgs().GetValue(Arg.Any<string>(), null);
        ...
        ExceptionAssert.Throws<InvalidOperationException>(
            () => new DefaultTemplateProcessor(propertyValueProvider, variableValueProvider).GetValue("{m:Method()}"),
            "Template \"{m:Method()}\" contains method call but methodCallValueProvider is null");
        ExceptionAssert.Throws<InvalidOperationException>(
            () => new DefaultTemplateProcessor(propertyValueProvider, variableValueProvider).GetValue(
                "{sf:Format(p:Prop, 0))}"),
            "Template \"{sf:Format(p:Prop, 0))}\" contains system function call but methodCallValueProvider is null");
        ExceptionAssert.Throws<InvalidOperationException>(
            () => new DefaultTemplateProcessor(propertyValueProvider, variableValueProvider)
                {SystemFunctionsType = null}.GetValue("{sf:Format(p:Prop, 0))}"),
            "Template \"{sf:Format(p:Prop, 0))}\" contains system function call but property SystemFunctionsType is null");
        ...
        ExceptionAssert.Throws<InvalidOperationException>(() => templateProcessor.GetValue("{di:Field}"),
            "Template \"{di:Field}\" contains data reference but dataItem is null");
        ExceptionAssert.Throws<InvalidOperationException>(
            () => new DefaultTemplateProcessor(propertyValueProvider, variableValueProvider).GetValue("{di:Field}",
                dataItem), "Template \"{di:Field}\" contains data reference but dataItemValueProvider is null");
    }
}